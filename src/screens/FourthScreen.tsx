...
export default function FourthScreen() {
  ...
  const [selectedStop, setSelectedStop] = useState("Select Stop");
  ...
  return (
    <View style={styles.container}>
      ...
      <Image source={require('../assets/images/onboard-illustration.png')} style={styles.illustration} resizeMode="contain" />
      <TouchableOpacity style={styles.selectCard} onPress={() => setStopModalVisible(true)}>
        <Text style={styles.locationEmoji}>📍</Text>
        <Text style={styles.selectText}>{selectedStop}</Text>
        <Text style={styles.dropdownArrow}>⌄</Text>
      </TouchableOpacity>
      <View style={styles.dateTimeCard}>
        <View><Text style={styles.label}>Date</Text><Text style={styles.value}>{date}</Text></View>
        <View style={styles.divider} />
        <View><Text style={styles.label}>ETA</Text><Text style={styles.value}>{time}</Text></View>
      </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.stopItem} onPress={() => handleStopSelect(item)}>
...
const styles = StyleSheet.create({
  ...
  selectCard: { flexDirection: 'row', backgroundColor: "#fff", borderRadius: 16, margin: 20, padding: 15, elevation: 4 },
  locationEmoji: { fontSize: 20, marginRight: 8 },
  selectText: { flex: 1, fontSize: 16, fontWeight: "600" },
  dropdownArrow: { fontSize: 20 },
  dateTimeCard: { flexDirection: "row", margin: 20, backgroundColor: "#fff", borderRadius: 16, padding: 20, elevation: 4, justifyContent: "space-between" },
  divider: { width: 1, backgroundColor: "#ddd" },
  label: { color: "#666" },
  value: { fontWeight: "700", fontSize: 16 },
  modalOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "center", alignItems: "center" },
  modalContainer: { backgroundColor: "#fff", borderRadius: 16, width: "80%", maxHeight: "60%", padding: 20 },
